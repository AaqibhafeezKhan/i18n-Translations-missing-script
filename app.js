...
function runDemo() {
  ...
  const ptRaw = document.getElementById("editor-pt").value;
  const nlRaw = document.getElementById("editor-nl").value;
  const jaRaw = document.getElementById("editor-ja").value;
  const zhRaw = document.getElementById("editor-zh").value;
  ...
  const pt = parseJSON(ptRaw);
  const nl = parseJSON(nlRaw);
  const ja = parseJSON(jaRaw);
  const zh = parseJSON(zhRaw);
  ...
  const translations = { en, de, fr, it, es, pt, nl, ja, zh };
  ...
  const pct =
    totalKeys > 0 ? ((missed.length / totalKeys) * 100).toFixed(1) : "0.0";
